...
//        setMaxWidth(size);
	...
	private void setLocation(Location l) {
		...
		markerY = location.getMapYcord() - (int) (size * 1.5);
		unscaledX = markerX;
		unscaledY = markerY;
	...
	public void onClick(View v) {
		...
		if (clicked) {
            ...
            LocationParcel parcel = new LocationParcel(l.getMap().getMapName(),
                    l.getMap().getMapURL(), l.getMapXcord(), l.getMapYcord(),
                    l.getAccuracy(), l.getSymbolicID(),
                    l.getRemoteId());
            intent.putExtra("location", parcel);
            intent.setAction(MapViewActivity.REPICK_INTENT);
