	...
	private static void updateCurrentModel(Context ctx) {
		...
		if (mCurrentLineNumber < 0) { 
			...
			Cursor c = db.rawQuery("SELECT * FROM " + DatabaseAPI.MyCounter.TABLE_NAME +
					" ORDER BY " + DatabaseAPI.MyCounter.VALUE_TS + " LIMIT 1 OFFSET " + mCurrentLineNumber, null);
			...
			if (c.moveToFirst()) {
				...
				mCurrentModel.setTS(c.getString(c.getColumnIndex(MyCounter.VALUE_TS)));
				MyLog.d("Debug", "Selected value among " + c.getCount() + " in DB where BYTES = " + mCurrentModel.getBytes()
